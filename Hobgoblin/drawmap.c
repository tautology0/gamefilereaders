...
char **argv;
{
   ...
   infile=fopen("$.HOB-2","rb");
   strncpy(outfile,"hobgoblin-map",255);
   
   if (infile == NULL)
   {
      printf("Could not load file %s\n",argv[1]);
   }
   fread(workarea,0x1b90,1,infile);
   fclose(infile);
   ...
   BITMAP *bigmap=create_bitmap(24*24,8*16*16);
   char filename[256];
   ...
   for (x=0; x<365; x++)
   {
      ...
      for (y=0; y<8; y++)
      {
         ...
         i &= 0x3f;
         printf("Sprite %d %d\n",i,yoff);
         draw_sprite(bigmap, sprites[i], (x % 24)*24, ((yoff*8)+y)*16);
      }
   }
